...
import com.lansmancai.lanhall.chatroom.client.ChatIndex;
import com.lansmancai.lanhall.client.exception.ClientException;
import com.lansmancai.lanhall.client.util.ImageUtil;
...
public class  LoginFrame extends JFrame {
	...
	private void login() {
		...
		setUser();
		setUsocket();
		//StoreSocket.setuSocket(usercreateSocket(this.connectionField.getText(), 12000);
		//得到用户所选择的游戏
		Game game = (Game)this.gameSelect.getSelectedItem();
		game.start(this.user, this.usocket);
		//启动线程
		ClientThread thread = new ClientThread(this.user, this.usocket);
		thread.start();
	...
	private Socket getUsocket() {
		return usocket;
	}
	private void setUsocket() {
		usocket = createSocket(this.connectionField.getText(), 12000);
	}
	private int getSex() {
